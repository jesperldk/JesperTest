...
public class DbFile<DBO> {
	...
	static public <D> DbFile<D> create(Class<D> cls, boolean fixedDboSize, Path path, DbContext context) {
		...
		return Observable.create(s -> {
			try ( 
				...
				while (p > headerSize) {
					...
					s.onNext((DBO) serializer.in(map,context));
				}
				s.onCompleted();
			} catch (Exception e) {
		...
		return new Observer<List<DBO>>() {
			...
			@Override public void onNext(List<DBO> t) {
				...
				t.forEach(dbo -> {
					...
					int p = buf.position();
					serializer.out(buf, dbo, context);
